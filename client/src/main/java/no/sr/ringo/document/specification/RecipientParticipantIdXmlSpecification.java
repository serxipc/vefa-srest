 ...
 */
public class RecipientParticipantIdXmlSpecification extends PeppolDocumentSpecification<ParticipantId> {
    ...
    public ParticipantId extractEntity(Element element) throws Exception {
        ...
        final SchemeId scheme = SchemeId.parse(schemeId);

        if (scheme == null) {
            return ParticipantId.valueOf(element.getText());
        }
        else {
            return new ParticipantId(scheme, element.getText());
        }
